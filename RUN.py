...
import matrix_utilizations as mu
import timeit
...
def init_population(coefficient_number, coefficient_bit_length, selected_basis_number, population_size):
    population = numpy.random.uniform(0.0, 1.0, [population_size,
                                                 coefficient_number * coefficient_bit_length + selected_basis_number])
...
def evaluate(population, space, coefficient_number, coefficient_bit_length, selected_basis_number, train_data,
    ...
    for k in range(population.shape[0]):
        print
        'begin evaluate %d/%d' % (k + 1, population.shape[0])
        pop = population[k]
        ...
        v = space.dot(coefficients)#a1
        null_v = mu.nul(numpy.asarray([v])).T
        weight = null_v.T  # each column is a basis
        selected_index = []
        for i in range(coefficient_number * coefficient_bit_length,
                       coefficient_number * coefficient_bit_length + selected_basis_number):
            if pop[i] == int(1):
        ...
        accuracy = mu.svm_predict(train_data, train_label, test_data, test_label, used_weight)
        fitness[k] = accuracy
        mse[k] = err
...
def genetic_operator(population, fitness, crossover_rate, mutation_rate, coefficient_number, coefficient_bit_length,
                     selected_basis_number, space, train_data, train_label, test_data, test_label):
    ...
    for i in range(offsprings.shape[0] - 1):
        ...
        if numpy.random.rand() < crossover_rate:
            ...
            change_point2 = numpy.random.randint(coefficient_number * coefficient_bit_length,
                                                 coefficient_number * coefficient_bit_length + coefficient_bit_length)
...
def evolve(train_data, train_label, test_data, test_label, population_size, generations, crossover_rate, mutation_rate):
    ...
    fitness, mse, weights = evaluate(population, space, coefficient_number, coefficient_bit_length,
                                     selected_basis_number, train_data, train_label, test_data, test_label)
    ...
    for gen in range(generations):
        ...
        print
        'begin current generation:%d/%d' % (gen + 1, generations)
        offsprings = genetic_operator(population, fitness, crossover_rate, mutation_rate, coefficient_number,
                                      coefficient_bit_length, selected_basis_number, space, train_data, train_label,
...
def train_autoencoder_one_layer_svm(train_data, train_label, test_data, test_label, svm_train_data, svm_train_label,
                                    svm_test_data, svm_test_label, best_weight, train_ae_epoch):
    ...
    accuracy = mu.svm_predict(svm_train_data, svm_train_label, svm_test_data, svm_test_label, model.get_weights()[0],
                              )
    best_weight = model.get_weights()[0]
    return accuracy, best_weight


def train_autoencoder_one_layer_softmax(train_data, train_label, test_data, test_label, best_weight, num_class,
                                        train_ae_one_layer_softmax_epoch):
    input_dim = train_data.shape[1]
    hidden_dim = best_weight.shape[1]
    inputs = Input(shape=(input_dim,))
    encoder = Dense(hidden_dim, activation="relu")(inputs)
    ...
    model.fit(train_data, new_train_label, nb_epoch=train_ae_one_layer_softmax_epoch, batch_size=256,
              validation_data=[test_data, new_test_label],  verbose=0)
    accuracy = model.evaluate(test_data, new_test_label, batch_size=256, verbose=0)
    return accuracy[1], model.get_weights()[0]


def train_autoencoder_two_layer_softmax(train_data, train_label, test_data, test_label, best_weight1, num_class,
                                        train_ae_two_layer_softmax_epoch, best_weight2):
    input_dim = train_data.shape[1]
    ...
    new_train_label = np_utils.to_categorical(train_label, num_class)
    #record_callback = RecordBestAccuracy()
    model.fit(train_data, new_train_label, nb_epoch=train_ae_two_layer_softmax_epoch, batch_size=256,
              validation_data=[test_data, new_test_label],  verbose=0)
    accuracy = model.evaluate(test_data, new_test_label, batch_size=256, verbose=0)
    return accuracy[1]


def train_autoencoder_three_layer_softmax(train_data, train_label, test_data, test_label, best_weight1, num_class,
                                          train_ae_three_layer_softmax_epoch, best_weight3, best_weight2):
    input_dim = train_data.shape[1]
    hidden_dim1 = best_weight1.shape[1]
    hidden_dim2 = best_weight2.shape[1]
    ...
    new_train_label = np_utils.to_categorical(train_label, num_class)
   #record_callback = RecordBestAccuracy()
    model.fit(train_data, new_train_label, nb_epoch=train_ae_three_layer_softmax_epoch, batch_size=256,
              validation_data=[test_data, new_test_label], verbose=0)
    accuracy = model.evaluate(test_data, new_test_label, batch_size=256, verbose=0)
    return accuracy[1]


def run_one_layer(train_data, train_label, test_data, test_label, svm_train_data, svm_train_label, svm_test_data,
                  svm_test_label, population_size, generations, num_class, train_ae_one_layer_epoch,
                  train_ae_one_layer_softmax_epoch):
    ...
    best_fitness, best_weight, best_mse = evolve(svm_train_data, svm_train_label, svm_test_data, svm_test_label,
                                                 population_size, generations, crossover_rate=0.8, mutation_rate=0.01)
    ...
    print
    'GA:total seconds:%f' % (end_time - start_time)
    ...
    start_time = timeit.default_timer()
    best_ae_one_layer_svm_accuracy, best_ae_one_layer_weight = train_autoencoder_one_layer_svm(train_data, train_label,
                                                                                               test_data, test_label,
                                                                                               svm_train_data,
                                                                                               svm_train_label,
                                                                                               svm_test_data,
    ...
    end_time = timeit.default_timer()
    print
    '1_AE_SVM:total seconds:%f' % (end_time - start_time)
    ...
    start_time = timeit.default_timer()
    best_ae_one_layer_softmax_accuracy, best_ae_one_layer_softmax_history_best, softmax_best_weight = train_autoencoder_one_layer_softmax(
        train_data, train_label, test_data, test_label, best_weight, num_class, train_ae_one_layer_softmax_epoch)
    numpy.savetxt('1_AE_SOFTMAX_best_acc.txt',
                  numpy.asarray([best_ae_one_layer_softmax_accuracy, best_ae_one_layer_softmax_history_best]))
    numpy.savetxt('1_AE_SOFTMAX_best_weight.txt', numpy.asarray(softmax_best_weight))
    end_time = timeit.default_timer()
    print
    '1_AE_SOFTMAX:total seconds:%f' % (end_time - start_time)
    logger.info('End for one layer auto-encoder and softmax, time %f' % (end_time - start_time))


def run_two_layer(train_data, train_label, test_data, test_label, svm_train_data, svm_train_label, svm_test_data,
                  svm_test_label, population_size, generations, num_class, train_ae_two_layer_epoch,
                  train_ae_two_layer_softmax_epoch):
    ...
    end_time = timeit.default_timer()
    print
    '2_AE_SVM:total seconds:%f' % (end_time - start_time)
    ...
    best_ae_one_layer_softmax_accuracy, best_ae_one_layer_softmax_history_best = train_autoencoder_two_layer_softmax(
        old_train_data, train_label, old_test_data, test_label, best_weight1, num_class,
        train_ae_two_layer_softmax_epoch, best_weight)
    ...
    numpy.savetxt('2_AE_SOFTMAX_best_acc.txt',
                  numpy.asarray([best_ae_one_layer_softmax_accuracy, best_ae_one_layer_softmax_history_best]))
    end_time = timeit.default_timer()
    print
    '2_AE_SOFTMAX:total seconds:%f' % (end_time - start_time)
    logger.info('End for two layer auto-encoder and softmax, time %f' % (end_time - start_time))


def run__three_layer(train_data, train_label, test_data, test_label, svm_train_data, svm_train_label, svm_test_data,
                     svm_test_label, population_size, generations, num_class, train_ae_three_layer_epoch,
                     train_ae_three_layer_softmax_epoch):
    ...
    start_time = timeit.default_timer()
    best_ae_three_layer_svm_accuracy, best_ae_three_layer_weight = train_autoencoder_one_layer_svm(train_data,
                                                                                                   train_label,
                                                                                                   test_data,
                                                                                                   test_label,
                                                                                                   svm_train_data,
    ...
    end_time = timeit.default_timer()
    print
    '3_AE_SVM:total seconds:%f' % (end_time - start_time)
    ...
    best_ae_three_layer_softmax_accuracy, best_ae_three_layer_softmax_history_best = train_autoencoder_three_layer_softmax(
        old_train_data, train_label, old_test_data, test_label, best_weight1, num_class,
        train_ae_three_layer_softmax_epoch, best_weight, best_weight2)
    numpy.savetxt('3_AE_SOFTMAX_best_acc.txt',
                  numpy.asarray([best_ae_three_layer_softmax_accuracy, best_ae_three_layer_softmax_history_best]))
    end_time = timeit.default_timer()
    print
    '3_AE_SOFTMAX:total seconds:%f' % (end_time - start_time)
...
if __name__ == '__main__':
    ...
    data = numpy.concatenate((data['mnist_train'],data['mnist_test']))
    train_data = data[0:35000,0:784]
    train_label = data[0:35000,784:]
    test_data = data[50000:62000,0:784]
    test_label = data[50000:62000,784:]
    svm_train_data = data[40000:50000,0:784]
    svm_train_label = data[40000:50000,784:]
    svm_test_data = data[35000:40000,0:784]
    svm_test_label = data[35000:40000,784:]
    num_class = numpy.unique(train_label).shape[0]
    print(num_class)
    # train_data = test_data
    # train_label = test_label
    # train the first layer
    run_one_layer(train_data, train_label, test_data, test_label, svm_train_data, svm_train_label, svm_test_data,
                  svm_test_label, population_size, generations, num_class, train_ae_one_layer_epoch,
                  train_ae_one_layer_softmax_epoch)
    # train the second layer
    run_two_layer(train_data, train_label, test_data, test_label, svm_train_data, svm_train_label, svm_test_data,
                  svm_test_label, population_size, generations, num_class, train_ae_two_layer_epoch,
                  train_ae_two_layer_softmax_epoch)
    # train the third layer
    run__three_layer(train_data, train_label, test_data, test_label, svm_train_data, svm_train_label, svm_test_data,
                     svm_test_label, population_size, generations, num_class, train_ae_three_layer_epoch,
                     train_ae_three_layer_softmax_epoch)
